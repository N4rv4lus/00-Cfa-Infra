...
Then Generate openbao web/api certificate : 
sudo openssl genpkey \
  -algorithm EC \
  -pkeyopt ec_paramgen_curve:P-384 \
  -pkeyopt ec_param_enc:named_curve \
  -out /etc/openbao/tls/openbao.superzone.com.key.pem

Send the request signature : 
sudo openssl req \
  -new \
  -sha384 \
...
verify the signature request (CSR): 
openssl req \
  -in /etc/openbao/tls/openbao.csr.pem \
  -noout \
  -text \
  -verify

Add to the CA intermediate the openbao role for openbao certificate :
nano /root/ca/intermediate/openssl.cnf

add this at the bottom of openssl.cnf
...
keyUsage = critical, digitalSignature
extendedKeyUsage = serverAuth
subjectAltName = @openbao_san

[openbao_san]
DNS.1 = openbao.lab.local
DNS.2 = openbao
IP.1 = 192.168.100.9

Then sign openbao certificate : 
sudo openssl ca -config /root/ca/intermediate/openssl.cnf   -extensions openbao_server   -days 365   -notext   -md sha384   -in /root/ca/intermediate/csr/intermediate.csr.pem -out /root/ca/intermediate/certs/openbao.cert.pem

then check the openBao Certificate after the intermediate signed it
openssl x509   -noout   -text   -in intermediate/certs/openbao.cert.pem

then validate the signature chain
openssl verify   -show_chain   -purpose sslserver   -verify_hostname openbao.lab.local   -CAfile /root/ca/certs/ca.cert.pem   -untrusted intermediate/certs/intermediate.cert.pem   intermediate/certs/openbao.cert.pem

openssl verify   -show_chain   -purpose sslserver   -verify_hostname openbao.superzone.com   -CAfile /root/ca/certs/ca.cert.pem   -untrusted intermediate/certs/intermediate.cert.pem   intermediate/certs/openbao.cert.pem
...
sudo chown root:openbao \
  /etc/openbao/tls/openbao.superzone.com.key.pem
...
sudo openssl req \
  ...
  -key /etc/openbao/tls/openbao.superzone.com.key.pem \
  -out /etc/openbao/tls/openbao.superzone.com.csr.pem \
  -subj "/C=FR/ST=France/O=Subarashi Corp/OU=oseille/CN=openbao.superzone.com" \
  -addext "subjectAltName=DNS:openbao.superzone.com,DNS:openbao,IP:192.168.100.9"
...
openssl x509 \
  -in /root/ca/intermediate/certs/openbao.superzone.com.cert.pem \
  -noout \
  -subject \
  -issuer \
  -dates \
...
openssl verify \
  -show_chain \
  -purpose sslserver \
  -verify_hostname openbao.superzone.com \
...
2 carte
  2.1 carte monde
    ...
    2.1.3 donjons
  2.2 cartes donjons
    2.2.1 deplacement dans la carte donjon
    2.1.2
3 histoire